...
def _calc_spanning_tree ():
  ...
  """
  print(" cal 111111111111111111")
  def flip (link):
    return Discovery.Link(link[2],link[3], link[0],link[1])
  #T = set()#defaultdict(lambda:defaultdict(lambda:[]))) 
    ...
    if False:
      ...
      for sw,ports in tree.iteritems():
        ...
        log.debug((" %i : " % sw) + " ".join([str(l[0]) for l in
                                             sorted(list(ports))]))
      log.debug("*********************")
    #T.add(tree) #defaultdict(lambda:defaultdict(lambda:[]))
  '''
...
def _handle_LinkEvent (event):
  ...
  _update_tree()
  print(" Event 111111111111111111")
...
def _update_tree (force_dpid = None):
  ...
  """
  print(" update tree 111111111111111111")
  # Get a spanning tree
...
def launch (no_flood = False, hold_down = False):
  ...
  def start_apsp ():
    ...
    print ("Spanning tree component ready   A")
  core.call_when_ready(start_apsp, "openflow_discovery")
